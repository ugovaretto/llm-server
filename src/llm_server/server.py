...
app = FastAPI(title="SOPA LLM Server", description="Stream Optimization and Performance Accelerator for LLMs")
...
class ResponseCache:
    ...
    def _make_key(self, messages: list, temperature: float, max_tokens: int, model: str) -> str:
        content = json.dumps({"messages": messages, "temp": temperature, "max_tokens": max_tokens, "model": model}, sort_keys=True)
    ...
    def set(self, messages: list, temperature: float, max_tokens: int, model: str, response):
...
class PerformanceMetrics:
    ...
    def record_request(self, latency: float, tokens: int, is_streaming: bool, is_error: bool = False):
...
def load_model(model_id: str, use_quantization: bool = False):
    ...
    if use_quantization:
        ...
        try:
            ...
            if hasattr(torch, 'cuda') and torch.cuda.is_available():
                ...
                from quark.quantization import QuantizationConfig
                quantization_config = {
                    "quantization_method": "quark",
                    "target_dtype": "fp8"
                }
                print(f"  ✓ AMD Quark quantization enabled")
            else:
            ...
            if use_quantization:
                print(f"  ⚠️  Required quantization backend not installed, skipping quantization")
    ...
    try:
        model = AutoModelForCausalLM.from_pretrained(
            model_name,
            device_map="auto",
            dtype=torch.bfloat16 if not use_quantization else None,  # quantization overrides dtype
...
async def chat_completion(request: ChatCompletionRequest, req: Request = None, api_key: Optional[str] = Depends(verify_api_key)):
    ...
    if request.model != model_name:
        ...
        raise HTTPException(status_code=422, detail=f"Invalid model. Expected: {model_name}")
    ...
    if not is_streaming and request.temperature < 0.3:
        cached_response = sopa_cache.get(request.messages, request.temperature, request.max_tokens, request.model)
        if cached_response:
            latency = time.time() - start_time
            sopa_metrics.record_request(latency, cached_response["usage"]["completion_tokens"], False)
    ...
    if is_streaming:
        ...
        if request.temperature < 0.3:
            sopa_cache.set(request.messages, request.temperature, request.max_tokens, request.model, response_data)
